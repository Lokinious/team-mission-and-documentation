# Startup Roadmap

## Planning assumption
This roadmap assumes a solo-founder pace with realistic sequencing, limited parallelism, and a preference for strong foundations before deeper scaling.

## Phased timeline

### Phase 1: Foundation setup
- Decide GitHub ownership and access model
- Create repository standards and starter templates
- Set finance and bookkeeping foundations
- Create first-pass legal and policy checklists
- Establish a lightweight branding foundation

### Phase 2: Repository scaffolding
- Create `main-ui`
- Create `agents`
- Create `infrastructure`
- Create `desktop-chess-game`
- Create `desktop-launcher`
- Add baseline CI, release, and security expectations

### Phase 3: First public presence and shared patterns
- Bootstrap `main-ui`
- Build the first public-facing page set
- Create the shared agent base template
- Create the first specialist agent prompts
- Define the publishing workflow for reviews, videos, and updates

### Phase 4: Shared platform groundwork
- Define backend and service direction
- Define the chess networking architecture
- Scaffold the launcher shell and install/update model
- Build the initial AWS and Terraform platform baseline

### Phase 5: Interactive prototypes
- Build the Tauri launcher proof of concept
- Build the Tauri chess prototype
- Build the RPG Maker RZ chess proof of concept
- Validate shared hosting and service assumptions

### Phase 6: Hardening and expansion
- Improve release, deployment, and operational safety
- Strengthen monitoring and support tooling
- Expand legal, privacy, and operational documentation
- Promote successful prototypes into more durable products

## Priority order
1. Foundation and standards
2. Repository creation and scaffolding
3. `main-ui` and `agents`
4. `infrastructure`
5. `desktop-launcher` and `desktop-chess-game`

## Practical note
If capacity gets tight, the best way to preserve momentum is to finish `main-ui`, complete the first agent set, scaffold infrastructure, and prioritize the Tauri chess path before treating RPG Maker RZ as a parallel experiment.
